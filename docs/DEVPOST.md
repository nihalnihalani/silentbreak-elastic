...
**The Google side.** The agent pipeline is built on the **Google Cloud Agent Builder stack: ADK 2.x** (the Agent Development Kit, now part of the Gemini Enterprise Agent Platform). A **`SequentialAgent`** orchestrates four `LlmAgent`s — Sentinel, RootCause, Guardian, Scribe — with **`gemini-3.5-flash`** as the model, chosen because it is Google's flagship agentic model and this is wall-to-wall tool calling. Sentinel, RootCause, and Scribe get Elastic's MCP tools via ADK's **`McpToolset`** over streamable HTTP; Guardian gets exactly one FunctionTool, gated on the human-approval token. Credentials work two ways: `GOOGLE_API_KEY`, or Vertex application-default credentials (`GOOGLE_GENAI_USE_VERTEXAI=1`) — the natural fit on Cloud Run.
...
**The serving layer.** A **FastAPI** app streams typed Server-Sent Events from a per-run EventBus (with buffer replay, so a finished examination is re-watchable) to a vanilla HTML/CSS/JS polygraph UI — no frameworks, zero external requests. The hosted demo runs on **Cloud Run** in mock mode (an in-memory Elasticsearch stand-in) so it needs zero secrets and is fully interactive for judges; real mode is configuration, not code. A second, deterministic engine runs the identical loop through the identical approval gate when no Gemini credentials are present, and the engine in use is always labeled honestly in the UI, the event stream, and the incident record.
